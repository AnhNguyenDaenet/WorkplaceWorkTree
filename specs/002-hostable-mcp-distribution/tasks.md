...
- [ ] T001 Verify baseline on branch `002-hostable-mcp-distribution`: run `npm ci`, `npm run lint`, `npm run build`, `npm test` — all green; record Node/npm versions and test counts in the task note; this state is the SC-006 reference
...
- [ ] T002 Create src/cli/args.ts: move workspace/max-doc-lines validation out of src/index.ts unchanged (same error texts, FR-006/FR-008), add `--http` (boolean), `--port` (int 1–65535, required with --http), `--host` (default `127.0.0.1`), `--version`/`-v` definitions via `node:util parseArgs`; export a validated `TransportConfiguration` per data-model.md §1
- [ ] T003 [P] Create src/transport/stdio.ts: extract the current StdioServerTransport connect + ready log from src/index.ts into `startStdio(server, config)` with byte-identical behavior
- [ ] T004 Rewrite src/index.ts as a thin router on `process.argv[2]`: no subcommand → serve (args.ts → createServer → stdio transport; HTTP branch stubbed until US2), `init` → stub exiting 1 with "not yet implemented", `--version`/`-v` → print version read from package.json and exit 0 (FR-015); legacy invocation `--workspace <p> [--max-doc-lines n]` MUST behave byte-identically to v0.1.0 (contracts/cli.md backward-compat guarantee)
- [ ] T005 [P] Contract test in tests/contract/cliArgs.test.ts: full flag table from contracts/cli.md — workspace required/absolute/exists errors unchanged, port range validation, "--port is required with --http", host defaults to 127.0.0.1, --version short-circuits before workspace validation
- [ ] T006 Regression checkpoint: run full suite — feature-001 unit/contract/integration/e2e tests pass with zero modifications to their files (FR-006, SC-006)
...
- [ ] T007 [P] [US1] Update package.json identity: name `@anhnguyendaenet/workspace-map-mcp`, version `0.2.0`, `publishConfig.access: "public"`, `repository`/`bugs`/`homepage` pointing to github.com/AnhNguyenDaenet/WorkplaceWorkTree, `keywords`, files `["dist","assets/grammars","README.md"]`; bin stays `workspace-map-mcp` (research R1)
- [ ] T008 [P] [US1] Create scripts/build-if-needed.mjs: skip compile when dist/index.js exists and is newer than the newest src/**/*.ts; otherwise run `tsc -p tsconfig.json`; exit 0 with a warning (never fail) when tsc is unavailable but a usable dist/ exists (research R2)
- [ ] T009 [US1] Wire lifecycle scripts in package.json: `prepublishOnly: "node scripts/fetch-grammars.mjs && tsc -p tsconfig.json"`, `prepare: "node scripts/fetch-grammars.mjs --soft && node scripts/build-if-needed.mjs"`; assert no `postinstall` exists (contracts/packaging.md lifecycle table, FR-001/FR-002)
- [ ] T010 [P] [US1] Create src/core/assets.ts: resolve assets/grammars/ via `new URL('../../assets/grammars/', import.meta.url)` from the compiled file, existence check with actionable error naming the expected directory, fallback probe one level up for tsx/source execution (FR-004, research R6)
- [ ] T011 [US1] Switch src/relations/parserRegistry.ts to the src/core/assets.ts resolver (remove its inline grammarsDir helper)
- [ ] T012 [US1] Pack-smoke + git-install test in tests/integration/packSmoke.test.ts: (a) tarball path — run `npm pack` programmatically → `npm install <tarball>` into a temp dir → verify no build ran on install (dist/ mtime unchanged) → execute installed bin `--version` → serve a copied fixture workspace via stdio client → `scan_structure` succeeds and grammars load (validates FR-001 + FR-004); (b) git path — `npm install git+file://<repo-root>` into a second temp dir (exercises the guarded `prepare` self-build exactly like a GitHub install, no network) → installed bin `--version` works and dist/ + grammars exist in the installed package (validates FR-002; together with (a) covers SC-002's registry+GitHub channels)
- [ ] T013 [P] [US1] Create CI workflow .github/workflows/ci.yml: triggers push/PR on `main` and `[0-9][0-9][0-9]-*`; steps checkout → setup-node 20 with npm cache → `npm ci` → `npm run lint` → `npm run build` → `npm test` → `npm pack` → upload tarball artifact (FR-011 validation half, contracts/packaging.md)
- [ ] T014 [US1] Channel checkpoint: locally verify global install path — `npm link` (or `npm i -g .`) → `workspace-map-mcp --version` from an unrelated directory → scan a sample project → `npm unlink -g` cleanup; record evidence in task note (US1-AS3, FR-003)
...
- [ ] T019 [P] [US3] Create Dockerfile (node:20-slim, WORKDIR /app, copy package.json + prebuilt dist/ + assets/grammars/, `npm ci --omit=dev --ignore-scripts`, `USER node`, `ENTRYPOINT ["node","dist/index.js"]`) and .dockerignore (node_modules, tests, specs, .git, .codemap, docs, src) per contracts/packaging.md (FR-010, research R7)
...
- [ ] T026 [P] Write docs/how-it-works.md: what the maps are, document formats, reading order for agents, architecture sketch (tools → core → render), staleness/update model
- [ ] T027 [P] Write docs/install.md: all four channels (npx registry, GitHub install, global/local link, Docker pull) with copy-paste commands and `--version` verification per channel (US1-AS3 documented command)
- [ ] T028 [P] Write docs/transports.md: stdio and HTTP recipes with full mcp.json snippets for VS Code and Claude Desktop, `--host` opt-in security note (localhost default, no auth v1, trusted networks only)
- [ ] T029 [P] Write docs/docker.md: stdio + HTTP run recipes, bind-mount rules, GHCR tags, container `--host 0.0.0.0` + `-p` exposure explanation
- [ ] T030 [P] Write docs/init-command.md: flag reference from contracts/cli.md, merge behavior + state table, examples per channel/transport, JSONC limitation note
- [ ] T031 [P] Write docs/troubleshooting.md: absorb and expand the current README troubleshooting section (workspace errors, slow scans, fallback tiers, stale maps, duplicate markers, port conflicts, grammar-resolution errors)
- [ ] T032 Rewrite README.md as a concise quickstart (≤ ~80 lines): what it is, npx + minimal mcp.json fastest path, tool table, `init` one-liner, link index into all six docs/ pages; every snippet uses `@anhnguyendaenet/workspace-map-mcp` (FR-014a, depends on T026–T031)
- [ ] T033 Final gate: `npm run lint`, `npm run build`, full `npm test` (incl. pack smoke; docker smoke where available) green; feature-001 suite untouched and passing (SC-006); `npm pack --dry-run` contents match contracts/packaging.md files list; walk quickstart.md §4 verification table and check every row
